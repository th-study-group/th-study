        ...
        <div class="board-card bg-white rounded-3 p-3 p-lg-4 shadow-sm">
            ...
            @if ($errors->any())
                <div class="alert alert-warning d-flex align-items-center gap-2 small mt-3" role="alert">
                    ...
                    <span>문의내역을 등록 실패 사유를 확인하세요.</span>
                </div>
            @endif

            <form id="form-inquiries" name="form-inquiries" class="board-form mt-3" method="post" action="{{ $isEdit ? route('inquiries.update', ['idx' => $post->idx]) : route('inquiries.store') }}">
                @csrf
                @if($isEdit)
                    @method('PUT')
                @endif
                ...
                <div class="mb-3">
                    ...
                    <textarea id="content"
                              name="content"
                              class="form-control board-textarea @error('content') is-invalid @enderror"
                              rows="8"
                              placeholder="문의 내용을 입력해 주세요">{{ old('content', $post->content ?? '') }}</textarea>
                    @error('content')
                        <div class="invalid-feedback d-block small text-break">{{ $message }}</div>
                    @enderror
                </div>
...
@section('script')
    <script>
        $(function(){
            ...
            $('#form-inquiries').on('submit', function(e) {
                if (isSubmitting) {
                    e.preventDefault();
                    return false;
                }
