                    ...
                    <div id="inquiryFilters" class="collapse show mt-2">
                        ...
                        <form id="form_search" name="form_search" method="GET" action="{{ route('admins.inquiries.index') }}">
                            <div class="row g-2 g-md-3">
                                ...
                                <div class="col-12 col-md-4">
                                    <label for="search_status" class="form-label small text-secondary mb-1">상태</label>
                                    <select id="search_status"
                                            name="search_status" 
                                            class="form-select form-select-sm">
                                        <option value="">전체</option>
                                        @foreach ($statusList as $statusValue => $statusLabel)
                                            <option value="{{ $statusValue }}" @selected(old('search_status', $filters['search_status'] ?? '') === $statusValue)>{{ $statusLabel }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-12 col-md-4">
                ...
                <table id="inquiry_table" class="table table-bordered table-hover align-middle mb-0 board-table">
                    ...
                    <thead class="table-light">
                        <tr class="text-center">
                            ...
                            <th scope="col" class="board-col-hidden text-nowrap">작성자</th>
                    ...
                    <tbody>
                        @forelse ($posts as $post)
                            ...
                            @endphp
                            <tr class="text-center inquiry-row" data-href="{{ route('admins.inquiries.show', ['idx' => $post->idx]) }}" role="link" tabindex="0" style="cursor: pointer;">
                                <td class="text-nowrap">{{ $number }}</td>
                                <td class="text-nowrap">
                                    <span class="badge text-bg-{{ $badgeClass }}">{{ $statusLabel }}</span>
                                </td>
                                <td class="text-start">
                                    <a href="{{ route('admins.inquiries.show', ['idx' => $post->idx]) }}" class="text-dark text-decoration-none board-ellipsis d-block inquiry-link">
                                        {{ $post->title }}
                                    </a>
                                </td>
