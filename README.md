...
- 블로그 목록의 상세 모달은 제목·작성일 아래, 본문 위에 `note_modal_top_display` 디스플레이 슬롯을 둡니다. 본문과 관련 글 사이에는 관련 글이 있을 때만 `note_modal_related_in_article` 인아티클 슬롯을 노출하고, 해시태그 바로 아래에는 `note_modal_tags_multiplex` 멀티플렉스 슬롯을 둡니다. 해시태그 아래 슬롯 뒤에는 모달 내부 스크롤 여유를 두어 Auto ads 앵커가 하단에 떠도 광고 전체를 앵커 위로 스크롤해 볼 수 있게 합니다. 앵커의 노출·위치는 사이트 코드가 아닌 AdSense Auto ads 설정에서 관리합니다. 애드센스 전용 래퍼와 `<ins>`에 `width: 100%`를 적용해 flex 레이아웃에서 폭 0으로 축소되지 않게 하고, 커스텀 모달이 열린 뒤 `public/js/blog.js`가 실제 폭을 확인해 0보다 클 때만 한 번 초기화합니다. 광고가 보이지 않으면 콘솔의 `[AdSense][Note Modal]` 로그에서 `offsetWidth`, `offsetHeight`, `data-ad-status`, `data-adsbygoogle-status`로 렌더링·미배정·미초기화 상태를 구분합니다.
- 블로그 목록의 더보기는 사용자 클릭으로 새 글 10건을 받은 경우에만 `blog_load_more_display` 디스플레이 슬롯을 새 묶음의 4~7번째 글 제목 바로 위에 1개 삽입합니다. 기존 슬롯은 갱신하지 않고, 새 `<ins>`가 실제 폭을 확보한 뒤 한 번만 초기화합니다.
...
- `tools/list`와 `tools/call`은 모두 `mcp/tool.json` 정의를 기준으로 동작하며, 각 tool은 `levels` 필드로 허용 계정 레벨을 명시
- `ToolRunner`는 tool 이름으로 정의를 찾은 뒤 로그인 계정의 `user.level`이 `levels`에 포함되는지 확인하고, 허용된 경우에만 내부 서브 요청으로 실제 Laravel 라우트에 전달
...
- 동일 출처의 일반 페이지 링크는 웹과 standalone PWA 모두 `common.js` 전역 로딩을 표시합니다. 해시·UI 토글·새 탭·다운로드·외부 링크·이미지 미리보기는 페이지 이동이 아니므로 제외합니다.
...
- 홈의 최신 블로그 5건은 `ContentCacheService`를 통해 60분 캐시합니다.
- 캐시 키는 `content:blog:home:public:v{version}:limit:5` 형식으로 생성합니다. 버전을 키에 포함해 캐시를 안전하게 교체합니다.
