...
- 목록의 상세/수정 이동은 `data-href`와 JavaScript만 사용하는 방식보다 실제 `<a href>` 링크를 기본 이동 수단으로 사용한다.
- 테이블 row 전체 클릭이 필요한 경우에는 제목 영역에 실제 링크를 두고, row 클릭은 보조 이동 방식으로 구현한다. 링크 클릭이 row 클릭 핸들러와 중복 실행되지 않도록 중첩 interactive 요소를 제외한다.
- 링크 스타일을 기존 화면과 동일하게 유지해야 하는 경우 `text-decoration-none`만 사용하지 않는다. 밑줄 제거와 글자색은 별도 속성이므로 필요한 경우 `text-dark` 등 색상 클래스를 함께 지정한다.
- JavaScript로 row 이동을 보조할 때는 `role="link"`, `tabindex="0"`, Enter/Space 키보드 동작을 적용하고, `window.location.assign()` 호출 전 중복 이동을 방지한다.
