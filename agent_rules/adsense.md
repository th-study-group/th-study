...
- 블로그 상세처럼 한 모달 안에서 콘텐츠만 바뀌는 화면은 모달 닫기, 다른 글 열기, 관련 글 전환을 AdSense 새로고침 사유로 사용하지 않는다. 기존 `<ins>` 슬롯을 교체하거나 재초기화하지 않고 유지한다. 글마다 새 광고 슬롯이 필요하면 실제 상세 URL로 페이지 이동해 새 페이지 로드에서 초기화한다. Kakao AdFit 등 다른 광고 네트워크의 동적 재호출 규칙을 AdSense에 그대로 적용하지 않는다.
- 블로그 목록의 더보기처럼 사용자가 명시적으로 새 콘텐츠를 요청한 경우에는 새 묶음에 `blog_load_more_display` 슬롯을 최대 1개만 독립 블록으로 삽입할 수 있다. 현재 `/blogs`는 새 10건 중 4~7번째 글 제목 바로 위에 배치한다. 기존 `<ins>`는 갱신하지 않고, 새 슬롯만 실제 폭이 0보다 큰 뒤 한 번 초기화한다. 목록 카드·제목·동작 버튼과 광고가 혼동되거나 오클릭이 생기지 않는 간격을 유지한다.
